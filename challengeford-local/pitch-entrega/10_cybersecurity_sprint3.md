...
| **SCA** | Dependabot + `npm audit` / OWASP Dependency Check mental model | CVEs em libs (Log4Shell-class) | Dependabot abre PRs semanais Java/npm/Actions |
| **SAST** | Semgrep (p/owasp-top-ten) | Injection XSS path traversal no código próprio | Scan `backend-java` + `mobile` + `microsservico-ia` |
| **Testes** | `mvn test` | Regressão auth (401/403) | Job `sast-and-tests` |
| **Container Security** | Trivy fs/image | Imagens com CVE críticos / misconfig Dockerfile | Scan Dockerfiles java/python/nginx |
